...
def parse_args():
    p = argparse.ArgumentParser(
        description="Train Titanic baseline model + log to MLflow",
        add_help=True)
...
def main():
    ...
    if not os.path.exists(args.data_path):
        raise FileNotFoundError(
            ...
            f"Pastikan CSV ada di folder MLProject/ (path relatif dari modelling.py) "
            f"atau jalankan dengan --data_path yang benar.")
    ...
    print(f"- accuracy={acc:.4f}, f1={f1:.4f}, auc={(auc if auc is not None else 'NA')}")
    print(f"- artifacts saved to: {extras_dir}")
